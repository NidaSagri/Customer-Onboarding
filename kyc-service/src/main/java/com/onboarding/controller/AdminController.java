...
public class AdminController {
    ...
    public String adminDashboard(Model model,
        ...
        long pendingApplications = kycApplicationRepository.countByKycStatus(KycStatus.PENDING);
        long verifiedApplications = kycApplicationRepository.countByKycStatus(KycStatus.VERIFIED);

        model.addAttribute("applications", applicationPage);
        model.addAttribute("keyword", keyword);
        model.addAttribute("searchActive", StringUtils.hasText(keyword));

        // 👉 Add analytics counts to the model
